...
# Se nao estiver vivo, faz nada aqui
	la t0, enemy4Alive
	lh t0, 0(t0)
	beq t0, zero, DamageEnemy4Ret
	
# Carrega a posição do inimigo4
	la t0, charPos
...
#testa se est� para direita
	li t1, 0
	la a0,inimigo4Direita				
	beq t0, t1, DrawEnemy4Continue
...
OkUpdateDirEnemy4:
	...
	la t0, enemy4Dir
	lb t0, 0(t0)
	bne t0, a0, NotSameEnemy4Dir

	# Testa se a dire��o � direita
	li t3, 16
	li t4, 0
	li t0, 0
	beq a0, t0, UpdateEnemy4Continue
...
UpdateEnemy4Continue:
	...
	li t1,0xFF000000			#Endere�o base
        ...
        bne t2,t5, UpdateEnemy4Ret		#se for parede, entao nao anda

	# Impede do inimigo pegar as chaves
        addi t1,t1,1				#pega exatamente a direita
	lb t2,0(t1)				#pega a cor que est� naquela posi��o
	bne t2, zero, UpdateEnemy4Ret

# Carrega as posi��es
	la t0,enemy4Pos				#carrega o endere�o da posi��o atual
        la t1,oldEnemy4Pos			#carrega o endere�o da posi�� antiga
...
# Altera a dire��o
	la t0,enemy4Dir				#carrega a dire��o atual
	sb a0,0(t0)				#salva a nova dire��o

	UpdateEnemy4Ret: ret

# Seta o oldEnem4Pos para o canto da tela para nao deixar nada piscando
HideOldEnemy4Pos:
	la t0, oldEnemy4Pos
	li t1, 304
	sh t1, 0(t0)
